...
beforeEach(() => {
  ...
  writeFileSync(executable, `#!/usr/bin/env node
    ...
    let sessionFile = "${directory}/session.jsonl";
...
describe("SessionManager", () => {
  it("creates a real RPC subprocess session and sends a prompt", async () => {
    const create = command("create-1", "session.create", null, { projectId: "anvil" });
  ...
  it("marks active runs and dialogs interrupted when Forge restarts", async () => {
    const created = await manager.handleCommand(command("create-interrupted", "session.create", null, { projectId: "anvil" }));
    const sessionId = (created.data as { sessionId: string }).sessionId;
    await manager.stopAll();
    events.append([
      {
  ...
  it("rejects model and thinking settings outside the session catalog", async () => {
    const created = await manager.handleCommand(command("create-capabilities", "session.create", null, { projectId: "anvil" }));
    const sessionId = (created.data as { sessionId: string }).sessionId;
  ...
  it("fails the session and resolves dialogs when Pi exits unexpectedly", async () => {
    const created = await manager.handleCommand(command("create-crash", "session.create", null, { projectId: "anvil" }));
    const sessionId = (created.data as { sessionId: string }).sessionId;
  ...
  it("cancels timed-out dialogs upstream so Pi can continue", async () => {
    const created = await manager.handleCommand(command("create-timeout", "session.create", null, { projectId: "anvil" }));
    const sessionId = (created.data as { sessionId: string }).sessionId;
  ...
  it("durably cancels pending dialogs when abort succeeds", async () => {
    const created = await manager.handleCommand(command("create-dialog", "session.create", null, { projectId: "anvil" }));
    const sessionId = (created.data as { sessionId: string }).sessionId;
  ...
  it("returns the stored response for a duplicate command id", async () => {
    const create = command("create-once", "session.create", null, { projectId: "anvil" });
