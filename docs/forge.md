...
Configured projects seed the trusted workspace registry. The authenticated owner can add another Forge-local directory from the Workspaces `+` control; Forge validates and canonicalizes the path, then persists it in SQLite across restarts. This is a privileged action because Pi receives full access inside that directory. The owner-authenticated `GET /api/v1/projects/directories` endpoint lists at most 200 direct real directory children of the canonical projects root for that picker, sorted by name; registered paths, symlinks, files, and special entries are excluded.
...
The owner-authenticated `GET /api/v1/pi/catalog` endpoint powers the Skills & extensions catalog. Forge scans the global `~/.pi/agent/skills` and `~/.pi/agent/extensions` directories with bounded depth and item counts, ignores symlinks, reads only bounded skill frontmatter, and returns display paths rather than absolute Forge paths.

The owner-authenticated `GET /api/v1/pi/agent-settings` endpoint powers the Agents & models page. It lists subagent definitions from the Pi subagents extension alongside the saved model alias list, each with an ETag. `PUT /api/v1/pi/agent-settings/agent` rewrites one agent file while preserving unknown frontmatter, and `PUT /api/v1/pi/agent-settings/models` replaces the `ocode-model-aliases.json` alias list. Writes run serially, reject stale ETags, stage through temporary files, and fail closed on symlinks, special files, or non-Linux containment. Subagent model fields store concrete Pi model IDs, so renaming an alias never rewrites agent assignments.

The owner-authenticated `GET /api/v1/pi/resources/content` route reads discovered skill and extension entry files. Extension source is read-only. Same-origin `/api/v1/pi/skills` routes create and mutate skills, and do not accept a resource kind. Forge binds reads to securely opened Linux file descriptors, caps editable text at 512 KiB, and fails closed when descriptor-based containment is unavailable. Skill operations run serially, reject stale ETags, stage writes before publishing them, and reject links or special files. Duplicate copies the complete skill package within limits of 256 files, 16 MiB, and eight nested directories. Creation uses restricted file and directory modes, and deletion removes the complete selected skill package. These routes do not provide general access to the Forge account's home directory.
